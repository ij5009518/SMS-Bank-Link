...
export async function sendContactNotificationEmail(params: ContactNotificationParams): Promise<boolean> {
  const { to, senderName, senderEmail, subject, type, message } = params;

  const safeSenderName = escapeHtml(senderName) || "Unknown";
  const safeSenderEmail = escapeHtml(senderEmail?.trim() || "not_provided");
  const safeSubject = escapeHtml(subject?.trim() || "(no subject)");
  const safeType = escapeHtml(type?.trim() || "contact");
  ...
  try {
    ...
    const { error } = await client.emails.send({
      from: fromEmail,
      to,
      subject: `[Contact:${safeType}] ${safeSubject}`,
      html: `<!DOCTYPE html>
<html lang="en"><head><meta charset="UTF-8"/><meta name="viewport" content="width=device-width,initial-scale=1.0"/></head>
<body style="margin:0;padding:0;background:#f8fafc;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;">
  <table width="100%" cellpadding="0" cellspacing="0" style="padding:24px 12px;background:#f8fafc;"><tr><td align="center">
    <table width="640" cellpadding="0" cellspacing="0" style="max-width:640px;width:100%;background:#fff;border:1px solid #e2e8f0;border-radius:14px;overflow:hidden;">
      ...
      <tr><td style="padding:24px;">
        <table width="100%" cellpadding="0" cellspacing="0" style="border-collapse:collapse;">
          <tr><td style="padding:6px 0;color:#64748b;width:120px;">Type</td><td style="padding:6px 0;color:#0f172a;"><strong>${safeType}</strong></td></tr>
          <tr><td style="padding:6px 0;color:#64748b;">Subject</td><td style="padding:6px 0;color:#0f172a;">${safeSubject}</td></tr>
          <tr><td style="padding:6px 0;color:#64748b;">Sender</td><td style="padding:6px 0;color:#0f172a;">${safeSenderName}</td></tr>
          <tr><td style="padding:6px 0;color:#64748b;">Email</td><td style="padding:6px 0;color:#0f172a;">${safeSenderEmail}</td></tr>
        </table>
        <hr style="border:none;border-top:1px solid #e2e8f0;margin:18px 0;"/>
        <p style="margin:0 0 8px;color:#475569;font-size:13px;font-weight:700;text-transform:uppercase;letter-spacing:0.04em;">Message</p>
        <div style="margin:0;padding:14px;background:#f8fafc;border:1px solid #e2e8f0;border-radius:10px;color:#0f172a;font-size:14px;line-height:1.6;white-space:normal;">
          ${safeMessageHtml}
        </div>
      </td></tr>
    </table>
...
<body style="margin:0;padding:0;background:#f8fafc;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;">
  <table width="100%" cellpadding="0" cellspacing="0" style="padding:24px;background:#f8fafc;">
    ...
    console.log("[Email] Contact notification email sent");
    return true;
  } catch (err) {
    console.warn("[Email] Could not send contact notification email:", err instanceof Error ? err.message : err);
