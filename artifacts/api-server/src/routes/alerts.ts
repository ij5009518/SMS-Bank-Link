...
router.patch("/:id", async (req, res) => {
  ...
  // Scope the update by userId so one user can't mutate another's alert.
  const [updated] = await db.update(alertsTable).set(updates)
    .where(and(eq(alertsTable.id, id), eq(alertsTable.userId, userId))).returning();
